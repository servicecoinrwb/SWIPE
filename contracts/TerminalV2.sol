...
interface IFeeSink {
    ...
    function earn(address merchant, uint256 amount) external;
}
 ...
 *
 * Merchants now earn escrowed units of the staking token as they process,
 * one per unit of volume, minted by the pool itself rather than handed out
 * by an operator. The first version could only distribute that token by
 * hand, which made "earned by using it" a promise instead of a mechanism.
 *
 * The earning call cannot break a payment. If it fails for any reason —
 * a supply cap reached, a pool that no longer accepts it — the payment
 * still settles and the reward is simply skipped. Payments are the
 * product; the reward is a perk, and a merchant losing a sale because a
 * loyalty mechanism hiccupped would be indefensible.
 */
contract TerminalV2 {
    ...
    /// @dev One escrowed unit per unit of volume. USDC carries six decimals
    ///      and the token eighteen, so the scale factor bridges them: a
    ///      hundred-dollar payment earns a hundred esSWIPE, which is the
    ///      only ratio anyone would guess without being told.
    uint256 public constant REWARD_PER_UNIT = 1e12;
    ...
    uint256 public rewarded;     // escrowed units minted to merchants
    uint256 public rewardsMissed;// volume that earned nothing, for honesty
    ...
    event Rewarded(address indexed merchant, uint256 amount);
    event RewardSkipped(address indexed merchant, uint256 amount);
    ...
    constructor(IERC20 token_, IFeeSink sink_, uint16 feeBps_) {
        ...
        sink = sink_;
        feeBps = feeBps_;
        // Approve the sink once, for everything. It can only ever pull what
        // this contract hands it inside a payment, and holding an allowance
        // avoids an approve on every single transaction.
        token_.forceApprove(address(sink_), type(uint256).max);
    }
        ...
        nonReentrant
    {
        ...
        if (fee > 0) {
            ...
            // If the sink ever reverts, the whole payment reverts with it.
            // Better a failed payment than a fee quietly stranded here.
            sink.depositFees(fee);
        }

        // The merchant earns escrowed units for the volume they brought.
        // Wrapped so a failure here can never cost anyone a sale — the
        // reward is skipped, recorded, and the payment stands.
        uint256 reward = amount * REWARD_PER_UNIT;
        try sink.earn(merchant, reward) {
            rewarded += reward;
            emit Rewarded(merchant, reward);
        } catch {
            rewardsMissed += amount;
            emit RewardSkipped(merchant, reward);
        }
    }
