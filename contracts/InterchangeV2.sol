 ...
 *
 * The first deployment could only be issued by hand, which meant the token
 * was handed out rather than earned — a weakness worth naming rather than
 * dressing up. Here a single payment rail can be authorised to mint
 * escrowed units directly, one per unit of volume it processes.
 *
 * That makes "earned by processing payments" a fact about the contract
 * instead of a claim about the operator's intentions. The rail is set once
 * and can never be changed, so the authorisation cannot be moved to
 * something else later.
 */
contract InterchangeV2 {
    ...
    /// @dev The one rail allowed to mint escrowed units by processing
    ///      volume. Set once, then permanently fixed — an authorisation the
    ///      operator could move later is just the operator minting.
    ...
    event Earned(address indexed merchant, uint256 amount, uint256 total);
    ...
    /**
     * @notice Name the payment rail that may mint escrowed units.
     * @dev Callable once, ever. There is deliberately no way to change or
     *      revoke it: the whole point of earning is that it does not depend
     *      on the operator continuing to allow it.
     *
     *      This is not a new trust assumption — the issuer can already mint
     *      escrowed units at will. It narrows their discretion rather than
     *      widening it.
     */
    ...
    /**
     * @notice Mint escrowed units to a merchant for volume processed.
     * @dev Only the rail may call this. Closing issuance stops the
     *      operator's discretionary minting but NOT this — earning is
     *      rule-based, and switching it off by hand would make the rule a
     *      courtesy. The supply cap still binds, and reaching it stops
     *      earning silently rather than breaking payments.
     */
    function earn(address merchant, uint256 amount) external {
        if (msg.sender != terminal) revert NotTerminal();
        if (merchant == address(0) || amount == 0) return;   // never revert a payment

        uint256 after_ = _supply + escrowSupply + amount;
        if (supplyCap != 0 && after_ > supplyCap) return;    // cap reached, stop quietly

        escrowOf[merchant] += amount;
        escrowSupply += amount;
        earnedTotal += amount;
        emit Earned(merchant, amount, earnedTotal);
        emit Issued(merchant, amount, true);
    }
    ...
    struct Book {
        ...
        uint256 earnedTotal;    // escrowed units minted by processing
        address terminal;
        bool    closed;
    }
