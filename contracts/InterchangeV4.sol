 ...
 * something.
 *
 * REFUNDS TAKE THE TOKEN BACK TOO
 *
 * When a merchant reverses a payment, the SWIPE they were minted for it is
 * burned. You do not keep the reward for a sale you undid.
 *
 * If they have already spent or staked it, the clawback takes what it can
 * and records the rest publicly as unrecovered. It never reverts, because
 * a customer getting their money back matters more than the supply figure
 * being exactly right — and hiding the gap would be worse than having one.
 *
...
contract InterchangeV4 {
    ...
    error NotEnoughToClaw(uint256 have, uint256 want);
    ...
    uint256 public clawedBack;
    /// @dev SWIPE that should have been reclaimed on a refund but couldn't
    ///      be, because the merchant had already moved it. Published rather
    ///      than hidden — it is the one place supply can drift above what
    ///      net volume would imply.
    uint256 public unrecovered;
    ...
    event ClawedBack(address indexed merchant, uint256 asked, uint256 taken, uint256 shortfall);
    ...
    /**
     * @notice Reclaim minted SWIPE when a merchant refunds a payment.
     * @dev Takes what it can from their spendable balance and records any
     *      shortfall rather than reverting. A refund must never fail because
     *      of a token mechanism — the customer getting their money back
     *      matters more than the supply being exactly right.
     */
    function clawback(address merchant, uint256 amount) external returns (uint256 taken) {
        if (msg.sender != terminal) revert NotTerminal();
        if (merchant == address(0) || amount == 0) return 0;

        uint256 have = _bal[merchant];
        taken = have < amount ? have : amount;

        if (taken > 0) {
            unchecked { _bal[merchant] = have - taken; }
            _supply -= taken;
            mintedToMerchants = mintedToMerchants > taken ? mintedToMerchants - taken : 0;
            clawedBack += taken;
            emit Transfer(merchant, address(0), taken);
        }
        uint256 short = amount - taken;
        if (short > 0) unrecovered += short;
        emit ClawedBack(merchant, amount, taken, short);
    }
    ...
    struct Book {
        ...
        uint256 clawedBack; uint256 unrecovered;
    }
    ...
    function book() external view returns (Book memory b) {
        ...
        b.terminal = terminal;
        b.clawedBack = clawedBack;
        b.unrecovered = unrecovered;
    }
